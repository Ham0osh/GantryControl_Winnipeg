...
if platform.system() == "Windows":
    if "64 bit" in platform.python_compiler():
        ...
        setattr(
            _gclibo, "GArrayDownloadFile", getattr(_gclibo, "_GArrayDownloadFile@8")
        )
        ...
        setattr(
            ...
            getattr(_gclibo, "_GProgramDownloadFile@12"),
        )
        setattr(_gclibo, "GSleep", getattr(_gclibo, "_GSleep@4"))
        setattr(
            _gclibo, "GProgramUploadFile", getattr(_gclibo, "_GProgramUploadFile@8")
        )
        setattr(_gclibo, "GTimeout", getattr(_gclibo, "_GTimeout@8"))
        setattr(_gclibo, "GVersion", getattr(_gclibo, "_GVersion@8"))
        setattr(
            _gclibo, "GSetupDownloadFile", getattr(_gclibo, "_GSetupDownloadFile@20")
        )
        ...
        setattr(
            _gclibo, "GRemoteConnections", getattr(_gclibo, "_GRemoteConnections@8")
        )
...
class py:
    ...
    def GIpRequests(self):
        ...
        for line in str(self._buf.value.decode(_enc)).splitlines():
            ...
            ip_req_dict[fields[0] + "-" + fields[1]] = fields[
                2
    ...
    def GArrayUpload(self, name, first, last):
        ...
        _rc(
            _gclib.GArrayUpload(
                self._gcon, c_name, first, last, 1, self._buf, _buf_size
            )
        )  # 1 is comma delimiter
